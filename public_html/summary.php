...
ob_start();
session_start();
$affiliate_sales_status = $_SESSION["affiliate_sales"];
?>
<!DOCTYPE html>
<html lang="en">
    ...
    function swalFun() {
      ...
      if(isset($_SESSION["products_list"]) && isset($_SESSION["user_details"]) && isset($_SESSION["address_details"]) && isset($_SESSION["subscription_plan"])) {
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
          ...
          $_SESSION["amount"] = (($cost_total*$sub_plan)*12);

          $sql = "INSERT INTO orders (id, user_id, quantity, amount, subscription_plan, exipry_date, order_status) 
          VALUES ('".$order_id."', '".$user_id."', '".$quantity_total."', '".(($cost_total*$sub_plan)*12)."', '".$sub_plan."', '".$expiry_date."', 'dropped')";
          ...
          if(isset($referral_code) && $referral_code != "" && $referral_code != " " && $affiliate_sales_status != "YES"){
            ...
            if (mysqli_num_rows($result) > 0) {
              ...
              echo "swal('Oops!', 'Code Expired / Invalid', 'error'); \n";
                ...
                if (isset($referral_code) && $referral_code != "" && $referral_code != " " && $affiliate_sales_status == "YES") {
                    ...
                    $sp.= "swal('Success!', 'Everything is Good! \\n Kindly wait for 5 seconds, we are redirecting you to payment page.', 'success'); \n";
                    echo $sp;
                }
          }
        }
        ...
        <div class="section_one section " id="app">
            ...
            <table>
              ...
              if(isset($_SESSION["products_list"])) {
                ...
                $total_cost_for_sp = $total_cost_for_sp*(12)*($_SESSION["subscription_plan"]);
                $cookie_name = "amount";
                $cookie_value = $total_cost_for_sp;
            ...
            <h2>Total No.of pads = <span class="nopad"><?php if($total_pads != ""){ echo $total_pads; } ?>/month</span></h2>
            <h3>Cost of subscription for  <?php if(isset($_SESSION["subscription_plan"])){ $sp = $_SESSION["subscription_plan"]; echo $sp; } ?> years ₹ <span class="odometer" id="amnt"><?php if($total_cost_for_sp != ""){ echo $total_cost_for_sp; } ?></span></h3>
            <h4>Do you have a referral code? 
              <input class="cc" type="text" placeholder="Enter code here" id="c_c" name="cc" />   
            ...
            <transition name="custom-classes-transition" enter-active-class="animated bounceInLeft" leave-active-class="animated bounceOutRight">
                    <p class="mt-3" v-if="errr" style="color: red; margin-bottom: 20px" >Please enter the coupon code</p>
                    <p class="mt-3" v-if="suc" style="color: green; margin-bottom: 20px" >Coupon Code is Valid, Now Enjoy Extra 5% Discount.</p>
                    <p class="mt-3" v-if="inv" style="color: red; margin-bottom: 20px" >Entered Coupon Code is In-Valid</p>
                </transition>
              <button type="submit" class="btn btn-sm btn-primary mt-3" style="font-size: 12px !important; padding: 9px 15px !important;">Validate Referral Code</button>
            </form>
            
            
            <form action="http://www.sanionindia.com/summary.php"  method="post">
...
<script src="http://cdn.jsdelivr.net/npm/vue"></script>
    ...
    <?php
        $timeData = time();
        if($affiliate_sales_status == "YES") {
            echo "<script src='js/app_for_affiliate.js?".$timeData."'></script>";
        } else if ($affiliate_sales_status == "NO") {
            echo "<script src='js/app.js?".$timeData."'></script>";
        }
    ?>
</body>
</html>